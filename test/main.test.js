...
const csvdb = require('../modules/transformer');

describe('classic dataset', () => {
  let databaseConnection = null;
  before(done => {
    csvdb(path.join(__dirname, './fixtures/dataset.csv'))
      .then(db => {
  ...
  it('should find 2 records with an i in their first name', done => {
    databaseConnection
      .find(({ firstName }) => firstName.includes('i'))
      .then(records => {
        expect(records).to.have.lengthOf(2);
        done();
      })
...
describe('custom dataset', () => {
  let databaseConnection = null;
  before(done => {
    csvdb(path.join(__dirname, './fixtures/dataset_custom.csv'), {
      rtrim: true,
      delimiter: ';',
      comment: '#'
    })
